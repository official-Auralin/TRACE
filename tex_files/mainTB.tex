...
structure; how often that structure occurs in TempoBench's corpus is measured
in roadmap step~V4, and any nonempty key on an overdetermined instance already
separates the two objects. This collapse of strict counterfactual dependence
...
holds in general on (F2)--(F3) instances is open: a proof would make the
...
the difficulty knobs below. Measured grader runtimes over the targeted
$(|I|,k)$ ranges are roadmap step~V3.
...
of the machine, used in the worked examples below. The \emph{white-box} track
...
(Section~\ref{sec:difficulty}). A submission is read straight off the
...
human device one-to-one. Parity is the point: within each track, the human
and the agent receive \emph{the same information} and the same probe tool, take the
same actions, and are graded by the same checker. Nothing in the agent interface
...
training scale, with \emph{Credit} the costliest mode; measured grader
throughput over the targeted $(|I|,k)$ ranges is roadmap step~V3.
...
\begin{enumerate}[label=\textbf{V\arabic*.},leftmargin=2.6em]
  ...
  \item \textbf{Grader implementation and profiling.} Implement the checker of
        ...
        to. The five worked puzzles and the public TempoBench sample instance
        are already reproduced end-to-end from HOA artifacts by the reference
        checker (Section~\ref{sec:worked}, Conjecture~\ref{conj:corp}); this
        step extends that reference implementation to profiled scale.
  \item \textbf{Generator and corpus statistics.} Build the generator
        ...
        and the trivial-instance discard rate.
  \item \textbf{Black-box identifiability enforcement.} Implement the
        ...
        Report how restrictive the check is in practice.
