...
Nothing in iteration 11 broke level 4 — the grader faithfully implements the variant the paper picked, and that variant *rejects* the pair. The source is explicit (mainTB, "Why this is Halpern–Pearl, and which Halpern–Pearl"): it adopts the **original/updated** HP definition (contingencies may take non-actual values), under which overdetermination's causes are the two **singletons** and the pair fails AC3 minimality — and it states the road not taken in so many words: *"under the modified definition Puzzle 3's cause would be the joint pair {(0,a),(0,b)}, not the two singletons we report."* The player's submission was precisely the **modified-variant** answer. So this was neither a code bug nor a wrong instance: it was the grader silently enforcing a variant choice the player cannot perceive, and rejecting an answer the source itself certifies as the other legitimate HP reading. (*Resolved 2026-06-14, §20.18:* CORP's contingency discipline is now author-confirmed — the TempoBench keys were generated with **contingencies disabled**, i.e. the modified/joint-flip object; and the per-cell key is variant-invariant (Prop. `union`), so the variant choice affects only the witness granularity, not TempoBench's per-cell metric.)
...
For a shipped benchmark corpus, one variant must be declared per corpus (the paper keeps original/updated as its credit-assignment core, and uses CORP's confirmed contingencies-disabled object for TempoBench alignment — §20.18); dual acceptance was judged the right call for the *human game* at this iteration, where the variant distinction cannot be communicated nonverbally and both readings are spec-documented members of the HP family over the same device. The benchmark configurations section of the doc stands unchanged; this is a play-surface grading policy, recorded here as a deliberate, source-grounded deviation-with-justification rather than silent drift. *(Superseded by iteration 13: the mode-explicit construction dissolves the fork at the surface instead of splitting acceptance.)*
...
**Confirmed 2026-06-14 (TempoBench authors).** The TCE ground-truth keys were generated by **CORP with contingencies disabled** — exactly the contingency-free joint-flip object Credit·Joint grades. This settles the variant question (formerly open roadmap item V2). And the per-cell key is **variant-invariant** (new `mainTB` Prop. `union`: `union(minimal_actual_causes) == union(minimal_flip_sets)`, machine-checked in `verify_atomization.py`), so at TempoBench's per-cell granularity the keys equal `Γ^ac` on *every* instance; the original-vs-modified difference is purely witness-level grouping. All "pending V2 / predicted divergence / which variant" hedging is withdrawn (see `Verification/GateB/DEFINITIONAL_ALIGNMENT_REVIEW.md`).
