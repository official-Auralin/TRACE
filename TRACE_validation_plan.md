...
mainTB is unusually honest about its own conditionality — it states outright that *"every TempoBench-equivalence claim rests on (F1)–(F3) and Conjecture 1, to be discharged by roadmap steps V1–V2"* and that if either fails, TRACE "stands as an independent trellis-causation benchmark." A rigorous reviewer would accept the formal core (the finite specialization, Prop. pivotal, Prop. atomize, Prop. union, the three-object taxonomy) but would treat the following as **blocking** for any claim of TempoBench parity or benchmark validity: the unaudited F1–F3 assumptions, the absence of any HOA-derived worked instance (all five worked puzzles are hand-built), and the unmeasured grader complexity envelope. The HP-variant discipline is now **resolved** (CORP contingencies disabled, author-confirmed 2026-06-14; and the per-cell key is variant-invariant, Prop. union), leaving the CORP correspondence reduced to CORP's ω-regular flattening at corpus scale. None are fatal; all are checkable; the paper survives review only with V1–V3 discharged and claims conditioned accordingly.
...
> - **A2 definitional: RESOLVED (author-confirmed 2026-06-14)** — CORP uses Halpern's *modified* variant (output-resets, toggleable), and the shipped TCE keys were generated with **contingencies disabled** → the contingency-free joint-flip object. By Prop. union the per-cell key is variant-invariant, so it equals `Γ^ac` per-cell; Conjecture corp reduces to CORP's ω-regular flattening (`A2_definitional_note.md`).
> - **A2(ii) empirical: MEASURED(n=1), per-cell F1 = 1.0 both variants.** *Remaining (external):* full TCE dataset — N. Holzer re-preparing it. On arrival: drop JSONL in `GateA/external/`, re-run. The variant question is settled (contingencies disabled + per-cell variant-invariance), so the dataset is needed only to confirm CORP's ω-regular flattening at corpus scale, not to discriminate variants.
> - **A3: PASS + decision record issued.** Benchmark corpora declare ONE `hp_variant` (TempoBench-alignment ⇒ `modified`, per A2 definitional; credit-assignment stance ⇒ `original`; the alignment default is finalized at A2(ii)). *Update (2026-06-12):* with the mode-explicit construction adopted (see Gate C status), the canonical human CREDIT IT surface poses only the original/updated object as a demonstrated decider — dual acceptance is retired from all shipped surfaces (the iteration-12 dual-acceptance build is removed; git history preserves it).
> - **A4: PASS — RESOLVED in production:** Mealy edge-rendering shipped (the shared render core `GateB/render.py` draws and tests the column-k cells the device reads); any remaining undrawn cell is proven effect-irrelevant per instance by Gate C P_CAND (release-blocking CI).
>
...
- **Problem (now largely resolved).** The variant question is settled: CORP's keys are generated with **contingencies disabled** (author-confirmed 2026-06-14) — the contingency-free joint-flip object — and the per-cell key is variant-invariant (Prop. union), so it equals `Γ^ac` per-cell. What remains of the alignment claim is CORP's ω-regular cause flattening, checkable on the full dataset.
...
>
> **STATUS (2026-06-12) — mode-explicit construction adopted.** The Visible-Verification construction (`TRACE_puzzle_construction_proposal.md`, now marked ADOPTED) is implemented and verified: canonical human game = `TRACE_stop/pin/credit.html` (no embedded keys; live verdicts proven equal to the oracle for every possible player action by `verify_modes.py`); deterministic mode assignment + versioned quality gate for arbitrary inputs (`mode_assign.py`, `build_from_inputs.py`, `verify_mode_assignment.py`); P-SUB parity now audited **per variant policy** (original / modified / legacy union). **Gold-12 pipeline coverage** is part of the PASS condition: the twelve instances the retired single-board build shipped (P1–P5, TB, G1–G6 at seed 23) are regenerated deterministically (`gold_inputs.py`), built through the mode pipeline (`TRACE_auto_*.html`), and every input is either emitted-and-oracle-verified or rejected with a logged legibility reason (G3/G5/G6 fail the fan-out cap — the boards the peer reviews called illegible). The D1 pilot covers the P3/P4/P5 strata and includes the budgeted white-box arm (C2).
