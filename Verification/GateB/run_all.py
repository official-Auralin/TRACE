...
def main():
    ...
    lines = ["# Gate B — Results Report", "",
      ...
      f"- Original-vs-modified HP variant keys diverge (at WITNESS granularity) on {corp['original_vs_modified_divergence_count']}/{corp['counts']['retain']} retained instances (~{corp['original_vs_modified_divergence_count']/corp['counts']['retain']:.0%}) — the overdetermined class. This is the singleton-vs-joint grouping only; the per-cell union is variant-invariant (Prop. union), so it does NOT affect the CORP correspondence (TempoBench grades per-cell; CORP keys use contingencies disabled).",
