...
Run: 2026-06-12T21:36:58.096836 · Python 3.10.12 (sandboxed Linux)
...
- Original-vs-modified HP variant keys diverge (at WITNESS granularity) on 63/107 retained instances (~59%) — the overdetermined class. This is the singleton-vs-joint grouping only; the per-cell union is variant-invariant (Prop. union), so it does NOT affect the CORP correspondence (TempoBench grades per-cell; CORP keys use contingencies disabled).
