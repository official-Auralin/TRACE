...
- **`main.tex` / `main2.tex`.** Necessity/sufficiency framework, HP actual causation explicitly
  disclaimed; audited clean (only the CORP characterization updated — see follow-up below).

## Follow-up (2026-06-14): TempoBench authors confirm CORP contingencies DISABLED

The TempoBench authors confirmed the TCE ground-truth keys were generated by **CORP with
contingencies disabled**. This resolves the variant question that the items above had
to hedge. Engine-verified consequences:

- Contingencies-disabled = the **contingency-free counterfactual** = the subset-minimal
  joint input-flip = `engine.minimal_flip_sets` = the Credit·Joint object. It diverges from
  original/updated **only at the witness granularity** (singletons vs. joint groups).
- **Per-cell union is variant-invariant** (new mainTB **Prop. union**, machine-checked in
  `verify_atomization.py`: `union(minimal_actual_causes) == union(minimal_flip_sets)`,
  exhaustive ≤4 cells + random 5–6). So at TempoBench's per-cell granularity the keys equal
  `Γ^ac` on *every* instance, overdetermined or not.
- **Conjecture corp** is therefore no longer variant-conditional: the setting is confirmed
  and the per-cell object is variant-invariant; the only residual is CORP's ω-regular
  flattening at corpus scale (a dataset matter, not the contingency question).

De-hedging applied (this pass): mainTB (abstract, which-HP paragraph, Conjecture corp,
roadmap status, status-of-claims, variant policy; + Prop. `union`), main.tex/main2.tex CORP
characterization, `A2_definitional_note.md` (rewritten as the author-confirmation record),
`A3_decision_record.md`, `a2_corp_compare.py`, GateA README, GateB `run_all.py`/REPORT,
the game docs, and the game UI. The earlier "predicted divergence on overdetermined /
modified pending / A2(ii) open / if it holds at all" language is withdrawn as a mistaken
consequence of the (now-resolved) toggle uncertainty.
