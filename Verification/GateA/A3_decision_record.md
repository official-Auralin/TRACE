...
|---|---|---|
| Human game (play surface) | ~~`either`~~ **superseded 2026-06-12**: the mode-explicit construction (proposal, ADOPTED) poses CREDIT IT as a *demonstrated decider* — the original/updated object only; extinguishing sets are never a Credit answer, so no dual policy governs any shipped surface (`either` is retired with the removed iteration-12 build; git history preserves it). | The original rationale (variant not communicable nonverbally) is dissolved rather than overruled: the demonstration gesture *shows* the variant instead of naming it. The dual-union facts remain verified (a3 T3). |
