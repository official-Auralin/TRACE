...
**Canonical artifacts (one authority per consumer).** The canonical human game is the **mode-explicit, live-verdict pages** `TRACE_stop.html` / `TRACE_pin.html` / `TRACE_credit.html` (built and cross-checked by `Verification/GateB/build_modes.py`; semantics proven equal to the oracle by `verify_modes.py`). They expose the paper's three modes (mainTB §modes) under the paper's verbs — STOP IT = *Prevent*, PIN IT = *Pin*, CREDIT IT = *Credit* — and embed **no answer key**: every verdict is computed on screen by running the build-verified board, every quantifier discharged visibly (tally / sweep / counterexample replay). `TRACE_play.html` is retained as a **single-board sandbox (archive build)** of the dual-acceptance iteration-12 design; it is labeled as such on the page, is not the benchmark surface, and is kept as a pilot-comparison arm. `index.html` lands on the mode pages.
...
The demo's junction graph was Moore-style (bulb depending on inputs `0..k−1` only) while the formal candidate window is `0..k`. Production adopted option (a), **Mealy edge-rendering**: `build_player.py`/`build_modes.py` render the output on the edge entering column `k`, so moment-`k` cells are drawn and testable wherever the device reads them (e.g. the TB arbiter's `(r,3)` junction). Residual safety is machine-checked per instance by Gate C's **P_CAND**: every cell not testable on the board is proven effect-irrelevant in every context, so no correct answer is human-inexpressible and the agent's larger nominal space adds only wrong answers. This invariant is release-blocking: any future generator change must keep P_CAND green (CI gate, §8).
...
```jsonc
{
  ...
  "hp_variant_policy": "original",     // ONE declared variant per corpus: "original" | "modified" (A3 decision record); "either" is legal only for the sandbox archive build
...
| Sandbox archive (`TRACE_play.html`) | witness | correct ∧ subset-minimal under **either** HP reading — the iteration-12 dual-acceptance policy, retained only as an archive/pilot-comparison arm with its defect stated on the page. Not a benchmark surface. |
...
2. Junction-graph ↔ HOA simulation equivalence (§2.2) — `build_player.py`/`build_modes.py` abort on divergence.
