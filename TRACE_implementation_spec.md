...
| Human game — CREDIT IT | Credit (witness, demonstrated; original/updated) | the player's own exhibited contingency pair-of-runs: bulb on with the claimed switch as recorded, off when flipped, robust under every subset-reset of the bench to the recording (AC2(b), counterexample replayed on failure). **This is the original/updated HP object only.** By the atomization theorem (mainTB Prop. `atomize`: original/updated minimal causes are *always* singletons on (F2)–(F3) instances — SMT-proven for every window ≤ 8 cells, `verify_atomization.py`), this surface loses **no** expressible answer; `build_modes.py`'s singleton assertion is an invariant check, not a filter discarding multi-cell originals (none exist). |
| Human game — CREDIT·JOINT | Credit (witness, modified variant) | the player names a minimal **group** of switches whose simultaneous flip (every other switch held at its recorded value) extinguishes the bulb, no proper subgroup sufficing (`engine.minimal_flip_sets`; built `build_joint.py`, cross-checked + re-verified on the emitted file `verify_joint.py`). The **modified** HP object, shown not named — the play surface for TempoBench-alignment corpora, and by Prop. `atomize` the *only* surface on which an irreducible multi-cell cause can be a winning answer. |
...
7. **The variant must be handled explicitly.** HP original/updated vs modified diverge exactly on overdetermination. The CREDIT IT surface poses only the original/updated object (the demonstrated decider) — and by the **atomization theorem** (mainTB Prop. `atomize`, SMT-proven for every window ≤ 8 cells) that object is *always* a single switch on (F2)–(F3) instances, so restricting to singletons loses nothing. Extinguishing (joint-flip) sets are the **modified** object; they get their own explicitly-declared surface, **CREDIT·JOINT** (`TRACE_credit_joint.html`) — the play surface for `modified`/TempoBench-alignment corpora and the only one where a genuine multi-cell cause can win — not dual-accepted on CREDIT IT. Benchmark corpora declare exactly one `hp_variant`; TempoBench-alignment corpora declare `modified` (A3). The iteration-12 dual-acceptance policy stays retired; the modified reading returns as a *separate surface*, not a second acceptance on one surface.
